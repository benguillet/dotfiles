...
4. This unit "changed" in this pass if step 2 rebased it OR step 3 left at least one surviving "fixed" commit. If it changed: run this repo's gate ONE FINAL time over the combined result (the rebase plus every surviving fix commit together — catches interactions between individually-green fixes, or a clean rebase that alone breaks something) and iterate until green. If it's still red after reasonable iteration: do NOT push this unit at all this pass — go back and change every "fixed" entry you recorded for this unit in step 3 to { action: "failed", reasoning: "gates red at final check (individually green, but combined state failed): <summary>" }, note it in "detail", and treat this unit as UNCHANGED for the next unit's rebase check in step 2 (nothing new landed on origin). If the final gate is green, proceed to step 5.
